...
echo "User Data Script Version: 3.1"
            ...
            "files": {
                "collect_list": [
                    ...
                    },
                    {
                        ...
                        "log_group_name": "${log_group_name}",
                        "log_stream_name": "worker-$(curl -s http://169.254.169.254/latest/meta-data/instance-id)"
                    }
                ]
            }
        }
...
log "=== Pulling images from ECR and starting containers ==="
if docker-compose pull && docker-compose up -d; then
    log "Docker Compose pulled and started containers successfully"
else
    log "Docker Compose failed to pull/start containers"
    docker-compose logs
    exit 1
fi
