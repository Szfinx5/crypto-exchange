...
export const executeTransactionsWorker = new Worker(
  ...
  async (job: Job<ExecuteTransactionsJobData>) => {
    ...
    console.log(chalk.blue(`Processing execute transactions for order: ${orderId}`));
    try {
        // Make the API call to execute transactions
      const executeTransactionsResponse = await executeTransactions(executeTransactionsRequest);
      if (!executeTransactionsResponse) throw new Error("Execute transactions response is null");
      console.log(chalk.green(`Execute transactions successful for order: ${orderId}`));

      // Update status to COMPLETED
      await setOrderStatus(orderId, Status.COMPLETED, statusRedis);
      return { executeTransactionsResponse };
    } catch (error) {
      console.error(chalk.red(`Execute transactions failed for order: ${orderId}`), error);
...
executeTransactionsWorker.on("completed", (job) => {
  console.log(chalk.green(`Execute transactions job ${job.id} completed for order: ${job.data.orderId}`));
});
executeTransactionsWorker.on("failed", (job, err) => {
  console.error(chalk.red(`Execute transactions job ${job?.id} failed for order: ${job?.data.orderId}`), err);
});
