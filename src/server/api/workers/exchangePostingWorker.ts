...
const workerRedis = new Redis(env.QUEUE_REDIS_URL, { maxRetriesPerRequest: null });
const statusRedis = new Redis(env.STATUS_REDIS_URL, { maxRetriesPerRequest: null });
...
export const exchangePostingWorker = new Worker(
  ...
  async (job: Job<ExchangePostingJobData>) => {
    ...
    console.log(chalk.blue(`Processing exchange posting for order: ${orderId}, attempt: ${job.attemptsMade + 1}`));
    try {
        // Update status to IN_PROGRESS and make the API call
      await setOrderStatus(orderId, Status.IN_PROGRESS, statusRedis);
      const exchangePostingResponse = await createExchangePosting(exchangePostingRequest);
      if (!exchangePostingResponse) throw new Error("Exchange posting response is null");
      console.log(chalk.green(`Exchange posting successful for order: ${orderId}`));

      // Format transaction currencies
      const [sellerCurrency, buyerCurrency] = getCurrenciesFromMarket(exchangePostingRequest.market);
      ...
      const executeTransactionsRequestBody = createExecuteTransactionsRequest(seller, buyer);
      ...
      return { exchangePostingResponse, executeTransactionsRequest: executeTransactionsRequestBody };
    } catch (error) {
      console.error(chalk.red(`Exchange posting failed for order: ${orderId}, attempt: ${job.attemptsMade + 1}`), error);
...
exchangePostingWorker.on("completed", (job) => {
  console.log(chalk.green(`Exchange posting job ${job.id} completed for order: ${job.data.orderId}`));
});
exchangePostingWorker.on("failed", (job, err) => {
  console.error(chalk.red(`Exchange posting job ${job?.id} failed for order: ${job?.data.orderId}`), err);
});
exchangePostingWorker.on("stalled", (jobId) => {
  console.warn(chalk.yellow(`Exchange posting job ${jobId} stalled and will be retried.`));
});
